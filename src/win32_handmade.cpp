...
Win32LoadXInput(){
	...
	if(!xinput_library) HMODULE xinput_library = LoadLibraryA("xinput9_1_0.dll"); //TODO diagnostic
	if(!xinput_library) HMODULE xinput_library = LoadLibraryA("xinput1_3.dll"); //TODO diagnostic
	if(xinput_library){
...
Win32MainWindowCallback(HWND window, UINT message, WPARAM w_param, LPARAM l_param){
	...
	switch(message){
		...
		case WM_KEYUP:{
			...
			b32 alt_key_was_down = l_param & (1 << 29); //checks if the 29th bit is set
...
Win32FillSoundBuffer(Win32SoundOutput* sound_output, GameSoundOutputBuffer* source_buffer, DWORD byte_to_lock, DWORD bytes_to_write){
	...
	if(SUCCEEDED(global_secondary_buffer->Lock(byte_to_lock, bytes_to_write, &region1, &region1_size, &region2, &region2_size, 0))){
		...
		s16* srcSample = source_buffer->samples;
...
WinMain(HINSTANCE instance, HINSTANCE prev_instance, LPSTR command_line, int show_code){
	...
	while(global_running){
		...
		GameUpdateAndRender(new_input, &render_buffer, &sound_buffer);
		...
		f32 mspf = (1000.0*(f32)counter_elapsed) / (f32)perf_count_frequency;
		f32 fps = (f32)perf_count_frequency / (f32)counter_elapsed;
		f32 mcpf = (f32)cycles_elapsed / (1000.0*1000.0);
